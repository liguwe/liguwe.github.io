...
`make.js` 会把已发布文章里实际引用的 `os/assets/` 本地资源复制到 `docs/public/assets/os/`，并把 Obsidian 资源嵌入改写成站点可访问路径。公开资源输出是扁平路径 `/assets/os/<文件名>`，不保留 `os` 内部的 `cos/YYYY/MM` 或 `files/` 目录层级。未发布稿和未引用资源不会进入公开站点仓库。
