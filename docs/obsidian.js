...
fs.writeFileSync("./src/part.json", JSON.stringify(part));
...
const findFirstFile = (dir) => {
  ...
  if (dir?.children?.[0]) {
    return findFirstFile(dir.children[0]);
  }
  return null;
};

// 递归遍历 part 的 children
const recursive = (arr) => {
  arr.forEach((item, index) => {
    // 如果是目录, 且不是 part
    if (item.type === "directory" && !item.isPart) {
      const firstFileItem = findFirstFile(item.children[0]);
      if (firstFileItem) {
        item = {
          ...firstFileItem,
          depth: item.depth,
