...
- **Claude.ai**（推荐）：适用于订阅计划用户。
- **Claude Console**：适用于使用 API 预付额度的用户。
- 登录后凭证会自动存储，无需重复登录。如果需要切换账户，可再次使用 `/login` 命令。

---

## 第三步：启动并探索项目

在你的项目根目录下启动 Claude Code，开始你的第一个会话。
...
启动后，你可以直接用自然语言向 Claude 提问，无需手动添加文件上下文，Claude 会自动读取文件并分析。
...
_注意：Claude Code 在修改文件前总是会请求你的许可。你可以逐个批准，也可以开启“全部接受”模式。_
...
> there's a bug where users can submit empty forms - fix it  # 修复用户可以提交空表单的 Bug
```
...
- **更新文档**：
	- `update the README with installation instructions`（更新 `README` 中的`安装`说明）
...
2. **分步说明**：将复杂任务分解。
    ...
    ```
3. **先探索，后修改**：在进行更改前，先让 Claude 理解代码。
...
希望这篇指南能帮助你顺利开始使用 Claude Code！如有疑问，可以在工具内输入 `/help` 获取帮助。
