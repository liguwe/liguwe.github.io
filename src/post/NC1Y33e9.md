...
反射型 XSS 是最简单的 XSS 攻击。攻击者将恶意代码拼接在 URL 中，服务器接收到请求后，将恶意代码"反射"回浏览器执行。
...
存储型 XSS 是最危险的 XSS 攻击。攻击者将恶意代码存储在目标服务器上（如数据库中），当其他用户浏览包含此恶意代码的页面时，就会执行攻击代码。
...
DOM 型 XSS 是一种特殊的 XSS，它的特点是攻击代码不需要服务器参与，而是通过修改页面 DOM 节点来完成攻击。
...
## 3. 一个完整的 XSS 攻击窃取用户信息的场景

### 3.1. 攻击场景设置

假设有一个社交网站 `social.example.com`，它有一个评论功能。用户可以发表评论，评论内容会被显示在页面上。
...
这个示例展示了一个完整的 XSS 攻击链，从漏洞利用到数据窃取的整个过程。它也说明了为什么正确的输入验证、输出编码和安全头部配置如此重要。在实际开发中，应该始终假设用户输入是不可信的，并采取适当的安全措施来防止此类攻击。