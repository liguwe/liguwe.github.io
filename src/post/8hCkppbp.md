...
前文 [8. Vue3 的响应式原理：effect、computed、watch 的实现原理](/post/wm0dSOlm.html) 注意讲了基本的响应式方案
...
- 数组是一个`异质对象`，因为数组对象部署的内部方法 `[DefineOwnProperty](/post/8hCkppbp.html#DefineOwnProperty)` 不同于常规对象。
...
- 集合类型指 Set、Map、WeakSet 以及 WeakMap
	- 例如，集合类型的 size 属性是一个**访问器属性**，当通过代理对象访问 size 属性时
		- 由于代理对象本身并没有部署 `[SetData](/post/8hCkppbp.html#SetData)` 这样的**内部槽**，所以会发生错误
...
前文 [8. Vue3 的响应式原理：effect、computed、watch 的实现原理](/post/wm0dSOlm.html) 的` Effect` 函数，如果对于下面的数据结构有问题，`无法正常收集响应信息`。这时候就需要用到 `Reflect 的第三个参数了`
...
所以，根据是否部署 `[Call](/post/8hCkppbp.html#Call)` 方法，就可以判断是 `普通对象` 还是`函数对象` 
...
- `[Call](/post/8hCkppbp.html#Call)` 和 `[construct](/post/8hCkppbp.html#construct)` 两个内部方法**只有被代理对象是函数和构造函数时**才会调用
- 内部方法的`多态性` 即 普通对象 和 Proxy 都有 `[Get](/post/8hCkppbp.html#Get)` ，但规范定义是完全不同的。
...
数组是`异质对象`，因为数组对象的 `[DefineOwnproperty](/post/8hCkppbp.html#DefineOwnproperty)` 内部方法与常规对象不同。
