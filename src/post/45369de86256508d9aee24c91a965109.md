...
- set 方法返回的是当前的Map对象，因此可以采用**链式写法** 
	- `0` 和 `-0` 就是**同一个键**
...
## 18. `Array.from()`方法用于将两类对象转为真正的数组
...
## 23. JSON.stringify 和 JSON.parse 可接受额外参数
