...
```bash
> Here's a screenshot of the error. What's causing it?  # 这是错误的屏幕截图。是什么原因导致的？
```
...
```bash
cat build-error.txt | claude -p 'concisely explain the root cause of this build error' > output.txt  # 简洁解释此构建错误的根本原因
```
...
```bash
cat data.txt | claude -p 'summarize this data' --output-format text > summary.txt  # 总结数据
```
