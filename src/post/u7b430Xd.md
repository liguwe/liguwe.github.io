...
> webpack 中使用模块联邦可参考 [10. Webpack 5 的 Module Federation（模块联邦）](/post/3ChRsUhe.html)
...
## 8. 示例：如何在 Vite 的 Module Federation 中共享 Pinia 状态

### 8.1. 首先需要在`主应用和远程应用`中都配置 Pinia 作为共享依赖
...
### 8.3. 主应用创建 Pinia 实例，并使用远程 Store

#### 8.3.1. 主应用中创建 Pinia 实例
...
### 9.1. 基础配置建议：vue 和 pinia 、 vue-router 必须单例
