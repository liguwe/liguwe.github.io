...
>  更多参考 [201.  分词器如何处理语言模型的输入](/post/v1jr6e1hjf.html)
...
>  更详细的运行说明，参考 [202.  下载和运行 LLM详细说明](/post/gvxozzi0gn.html)
...
>  关于 prompt 的格式，参考 [203. Phi-3 模型的完整指令说明](/post/30mi48jrkv.html)
...
>  更多细节参考 [204. 分词器如何分解文本：三个因素](/post/heg9wgeqba.html)
...
> 另参考 [205.  词级、子词级、字符级与字节级分词（文本的"最小处理单位"该如何选择）](/post/3bwut55gjk.html)

### 2.5. 市面上几种训练好的 LLM 分词器对比

> [!abstract]  
> 另外可参考，换了两种更直观的方式来描述这一节  
> 1、[206.  真实分词器测评（篇一：几种真实模型的 LLM 分词器的对比）](/post/jiqkf170yh.html)  
> 2、[207. 真实分词器测评（篇二）](/post/9co8nrmugx.html)
...
> [!abstract]  
> 更多参考  
> ① [208. 分词器的三要素决定论：算法、参数与领域数据（篇一）](/post/sc42y4a8kv.html)  
> ② [209. 分词器的三要素决定论：算法、参数与领域数据（篇二）](/post/u6zx3wpjp3.html)
...
> [!info]  
> 另外可参考
> - [211. 词云嵌入（精简版）](/post/8pntyv39vl.html)
> - [210.   词元嵌入（代码版本说明）](/post/wb78c8h11l.html)
> - [213. 词云嵌入（会编程的高中生版本）](/post/nkewsnpdy7.html)
> - [212.  词元嵌入（高中生版）](/post/rihx5hbwza.html)
...
> [!info]  
> 另外可参考
> - [216. 文本嵌入（用于句子和整篇文档）：代码详解篇](/post/uasgynv0u3.html)
> - [215. 文本嵌入（用于句子和整篇文档）：简洁篇](/post/isv6tsfj8w.html)
...
更多参考：
- [219. word2vec 核心原理：滑动窗口、二分类欺骗（及其解决）、负采样](/post/sgtjtcvmcg.html)
- [219. word2vec 核心原理：滑动窗口、二分、负采样](/post/jgvtdm4nqt.html)
