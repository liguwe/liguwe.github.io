...
看个例子，由 `vnode1` 变成 `vnode2` ，如果按照 [11. Vue3 的渲染器原理](/post/UBaXSq0T.html) 中的简易实现，即 `先卸载所有，再挂载所有`，那么：我们需要 `8 次 DOM 操作`
...
## 2. 简单 Diff 算法：考虑可复用 DOM ，减少 DOM 操作

还是看个例子，按照上面的优化策略，还是得 `6 次 DOM 操作` ，但是我们发现，其实每个节点的区别仅仅在于 `位置移动了`。所以我们需要考虑如何**复用** DOM，而不是`销毁了重建`
...
- 遍历新旧两组子节点中`数量较少`的那一 组，并逐个调用 patch 函数进行打补丁，然后比较新旧两组子节点的数量
