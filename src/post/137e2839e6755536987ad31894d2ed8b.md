...
> [!info]
> 渲染器的作用是，把虚拟DOM 对象渲染为真实 DOM元素。它的工作原理是，递归地遍历虛拟DOM对象，并调用原生DOMAPT来完成真实 DON 的创建。渲染器的精髓在于后续的更新，它会通过Diff算法我出变更点，并且只会更新需要更新的内容
...
### 5.2. `HTML Attributes` 和 `DOM Properities` 不一样

浏览器解析完以下 `HTML 代码段` 后，会创建以之相符的 `DOM 元素对象` ，比如
- `id="app"` 对应 `el.id`
