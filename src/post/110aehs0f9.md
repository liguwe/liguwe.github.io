...
>  更多参考 [201.  分词器如何处理语言模型的输入](/post/rrte352kvn.html)
...
>  更详细的运行说明，参考 [202.  下载和运行 LLM详细说明](/post/vic2uhx374.html)
...
>  关于 prompt 的格式，参考 [203. Phi-3 模型的完整指令说明](/post/0tm6znx7kj.html)
...
>  更多细节参考 [204. 分词器如何分解文本：三个因素](/post/bfwgt3w840.html)
...
> 另参考 [205.  词级、子词级、字符级与字节级分词（文本的"最小处理单位"该如何选择）](/post/onxiwredj7.html)

### 2.5. 市面上几种训练好的 LLM 分词器对比

> [!abstract]  
> 另外可参考，换了两种更直观的方式来描述这一节  
> 1、[206.  真实分词器测评（篇一：几种真实模型的 LLM 分词器的对比）](/post/q9sjmem54d.html)  
> 2、[207. 真实分词器测评（篇二）](/post/3ie7gyy9mi.html)
...
> - [213. 词云嵌入（会编程的高中生版本）](/post/p2yxl46eh6.html)
> - [212.  词元嵌入（高中生版）](/post/7zbrdgeli0.html)
...
> - [215. 文本嵌入（用于句子和整篇文档）：简洁篇](/post/zh6yfxpxnr.html)
...
更多参考：
- [219. word2vec 核心原理：滑动窗口、二分类欺骗（及其解决）、负采样](/post/6vliwm87ao.html)
- [219. word2vec 核心原理：滑动窗口、二分、负采样](/post/9apz5y7q3n.html)
